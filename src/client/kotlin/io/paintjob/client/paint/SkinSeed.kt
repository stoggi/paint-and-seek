...
object SkinSeed {
    ...
    private fun readResource(mc: Minecraft, id: net.minecraft.resources.Identifier): IntArray? {
        ...
        return try {
            ...
            Paintjob.LOGGER.warn("SkinSeed: resource read of '{}' failed", id, e)
            null
        }
    }
    ...
    private fun readFromGpu(mc: Minecraft, id: net.minecraft.resources.Identifier, onReady: (IntArray) -> Unit) {
        ...
        if (gpu.getWidth(0) != SkinImage.WIDTH || gpu.getHeight(0) != SkinImage.HEIGHT) {
            Paintjob.LOGGER.warn("SkinSeed: can't read skin texture '{}' from GPU — using grey base", id)
        ...
        val buffer = device.createBuffer({ "paintjob skin readback" }, 9, width.toLong() * height * blockSize)
        device.createCommandEncoder().copyTextureToBuffer(gpu, buffer, 0L, {
            try {
                ...
                Paintjob.LOGGER.warn("SkinSeed: GPU readback of '{}' failed — using grey base", id, e)
                onReady(greyBase())
            } finally {
                buffer.close()
            }
